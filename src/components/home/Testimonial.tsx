        ...
        <div className='flex flex-row space-x-1 flex-wrap'>
            {
                TesimoniItems.map((item)=> (
                    <div key={item.id} className={` ${active==item.id?'testimony_bg':''} flex flex-row rounded-[20px] space-x-2 p-2 `} onClick={()=>setActive(item.id)}>
                        <div>
                        ...
                        <AnimatePresence>
                            {
                                active==item.id && (
                                        ...
                                        <motion.div initial={{opacity: 0, width: 0}} animate={{opacity: 1, width: 357.89, transition:{duration: 0.5, delay: 0.2, ease: 'easeIn'}}} exit={{opacity: 0, width: 0, transition: {duration: 0.2, delay: 0.1, ease: 'linear'}}} className="glass bg-[#FDFAF8] border border-white self-stretch w-[357.89px] md:h-[392.47px] h-auto rounded-[20px] p-8">
