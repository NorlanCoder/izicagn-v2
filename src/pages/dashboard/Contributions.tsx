...
import { Calendar, ChevronDown, CheckCircle2, Clock, ExternalLink, Filter, XCircle } from "lucide-react"
import { useMyGiftsQuery } from "../../features/pot/mutations"

type ContributionStatus = "CONFIRMED" | "PENDING" | "CANCELLED" | "FAILED"
...
const STATUS_CONFIG: Record<ContributionStatus, { label: string; color: string; bg: string; Icon: React.ComponentType<{ className?: string }> }> = {
    ...
    FAILED: { label: "Échoué", color: "text-[#FF3B30]", bg: "bg-[#FFF0EF]", Icon: XCircle },
}
...
const DashboardContributions = () => {
    ...
    const { data, isLoading } = useMyGiftsQuery(1, 100)
    const gifts = data?.data ?? []

    const hasContributions = gifts.length > 0

    const totalDonne = gifts.filter((g) => g.state === "CONFIRMED").reduce((sum, g) => sum + Number(g.amount), 0)
    const donsEffectues = gifts.length
    const cagnottesSoutenues = new Set(gifts.map((g) => g.pot?.id).filter(Boolean)).size

    const filtered = gifts.filter((g) => {
        const status = (g.state ?? "PENDING") as ContributionStatus
    ...
    const filteredTotal = filtered.filter((g) => g.state === "CONFIRMED").reduce((sum, g) => sum + Number(g.amount), 0)
    const displayCurrency = filtered[0]?.currency ?? gifts[0]?.currency ?? "XOF"

    if (isLoading) {
        return (
            <DashboardLayout title="Mes contributions">
                <div className="flex justify-center items-center py-24">
                    <div className="w-8 h-8 border-4 border-[#07AED8] border-t-transparent rounded-full animate-spin" />
                </div>
            </DashboardLayout>
        )
    }
        ...
        <DashboardLayout title="Mes contributions">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 bg-[#110C2208] rounded-2xl p-2 gap-2 mb-8">
                <div className="flex items-center gap-3 bg-white rounded-2xl px-5 py-4" style={{ borderWidth: "1px 1px 3px 1px", borderStyle: "solid", borderColor: "#E8EDF2" }}>
                    ...
                    <div>
                        <p className="text-lg font-bold text-[#23C7ED]">{totalDonne.toLocaleString()} {displayCurrency}</p>
            ...
            <div className="bg-white border p-2 border-[#F0F2F5] rounded-2xl overflow-hidden">
                <div className="grid grid-cols-[2fr_1fr_1fr_1fr_auto] bg-[#FAFAFA] px-5 py-3 text-[11px] font-semibold text-[#8296A3] uppercase tracking-wider border-b border-[#F0F2F5]">
                    <span>Cagnotte</span>
                    <span>Montant</span>
                    <span>Date</span>
                    <span>Statut</span>
                ...
                {filtered.length === 0 ? (
                    <p className="text-center text-sm text-[#8296A3] py-10">Aucun don pour cette période.</p>
                ) : (
                    filtered.map((g) => {
                        const status = (g.state ?? "PENDING") as ContributionStatus
                        const cfg = STATUS_CONFIG[status]
                        const potTitle = g.pot?.title ?? "—"
                        const potImage = g.pot?.images?.[0] ?? ImgDeco
                        const giftCurrency = g.currency ?? "XOF"

                        return (
                            <div key={g.id} className="grid grid-cols-[2fr_1fr_1fr_1fr_auto] items-center px-5 py-4 border-b border-[#F0F2F5] last:border-b-0 hover:bg-[#FAFBFC] transition">
                                <div className="flex items-center gap-3 min-w-0">
                                    <img src={potImage} alt="" className="w-12 h-12 rounded-[10px] object-cover shrink-0" />
                                    <div className="min-w-0">
                                        <p className="text-sm font-semibold text-[#0E0E18] truncate">{potTitle}</p>
                                    </div>
                                </div>
                                <span className="text-sm font-bold text-[#FD8352]">{Number(g.amount).toLocaleString()} {giftCurrency}</span>
                                ...
                                <button
                                    onClick={() => g.pot?.id && navigate(`/cagnotte/details/${g.pot.id}`)}
                                    disabled={!g.pot?.id}
                                    className="flex items-center gap-1 text-xs font-semibold text-[#5A6272] border border-[#E5E7EB] rounded-lg px-3 py-1.5 hover:bg-[#F3F5F7] transition disabled:opacity-30"
                                >
                ...
                <div className="flex items-center justify-between px-5 py-3 border-t border-[#F0F2F5] text-sm">
                    <p className="text-[#0E405D]">
                        <span className="font-bold text-[#23C7ED]">{filtered.length} don{filtered.length > 1 ? "s" : ""}</span> affiché{filtered.length > 1 ? "s" : ""}
                    </p>
                    <p className="text-[#8296A3]">
                        Total : <span className="font-bold text-[#FD8352]">{filteredTotal.toLocaleString()} {displayCurrency}</span>
                    </p>
                </div>
            </div>
        </DashboardLayout>
